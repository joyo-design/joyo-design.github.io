let jdesign = function (newdata = null) {
    ...
    const method = {
        ...
        font: (family, source) => {
            ...
            if ($(`#${fontid}`).length == 0) {
                $("head").append(`<link id="${fontid}" href="${source}" rel="stylesheet">`);
            }
        },
        imagerecent(v = null) {
            if (v == null) {
            ...
            $.each(jd.pointer(pointer), (i, v) => {
                ...
                switch (type) {
                    ...
                    case `number`:
                        ...
                        break;
                    case `range`:
                        jd.pointer(pointer).eq(i).attr({ min: mn, max: mx }).val(value);
                        break;
                    default:
                        jd.pointer(pointer).eq(i).val(value);
    ...
    const event = [
            ...
            value: (v) => {
                ...
                switch (v) {
                    ...
                    case "copy":
                        ...
                        break;
                    case "reset":
                        data.display.element[data.display.key].layer[variable.layer] =
                            data.display.element[data.display.key].layer[variable.layer].type == "text" ? JSON.parse(`${JSON.stringify(variable.raw.text)}`) : JSON.parse(`${JSON.stringify(variable.raw.image)}`);
                        break;
                    case "delete":
        ...
        },
        {
            ...
            render: () => {
                ...
                $.each(data.product.color.recommendation, function (i, v) {
                    jd.class(`color-recommendation`).append(
                        `<button type="button" class="jd-color-recommendation" data-jd-class="input-click" data-jd-pointer="color" data-jd-value="${data.product.color.recommendation[i]}" style="background-color: ${data.product.color.recommendation[i]}">&nbsp;</button>`
                    );
                });
            },
        },
        ...
        },
        {
            ...
            value: (v) => {
                ...
                data.display.element[data.display.key].layer[variable.layer] = JSON.parse(`${JSON.stringify(style)}`);
            ...
            value: (v = null) => {
                ...
                $.ajax(settings).done(function (response) {
                    ...
                    $.each(response.items, function (i, v) {
                        if (
                            String(response.items[i].family).toLowerCase().includes(String(variable.font.keyword).toLowerCase()) &&
                            (variable.font.language.length == 0 ? true : response.items[i].subsets.includes(variable.font.language)) &&
                            (variable.font.category.length == 0 ? true : response.items[i].category.includes(variable.font.category))
                        ) {
                            items += `{"family": "'${response.items[i].family}', ${
                                response.items[i].category == "handwriting" || response.items[i].category == "display" ? "cursive" : response.items[i].category
                            }", "source": "https://fonts.googleapis.com/css?family=${String(response.items[i].family).replace(" ", "+")}&display=swap", "name":"${response.items[i].family}"},`;
                        }
                    });
        ...
        },
        {
            ...
            value: (v) => {
                ...
                if (data.display.element[data.display.key].layer[variable.layer].color.background.type == "normal" || data.display.element[data.display.key].layer[variable.layer].color.background.type == "same-parent") {
                    data.display.element[data.display.key].layer[variable.layer].color.background.main = [
                        data.display.element[data.display.key].layer[variable.layer].color.background.main[data.display.element[data.display.key].layer[variable.layer].color.background.main.length - 1],
                    ];
                }
                canvas.layer(variable.layer);
            },
            render: () => {
                ...
                data.display.element[data.display.key].layer[variable.layer].color.background.type != "normal" && data.display.element[data.display.key].layer[variable.layer].color.background.type != "same-parent"
                    ? jd.pointer(`text-coloradd`).css({ display: "block" })
                    : jd.pointer(`text-coloradd`).css({ display: "none" });
                data.display.element[data.display.key].layer[variable.layer].color.background.type == "linear-gradient"
                    ? jd.pointer(`text-colorposlinear`).css({ display: "block" })
                    : jd.pointer(`text-colorposlinear`).css({ display: "none" });
                data.display.element[data.display.key].layer[variable.layer].color.background.type == "radial-gradient"
                    ? $(`[data-jd-pointer='text-colorposrasialx'], [data-jd-pointer='text-colorposrasialy']`).css({ display: "block" })
                    : $(`[data-jd-pointer='text-colorposrasialx'], [data-jd-pointer='text-colorposrasialy']`).css({ display: "none" });
            },
        },
        {
            pointer: "text-color",
        ...
        },
        {
            ...
            value: (v = null) => {
                data.display.element[data.display.key].layer[variable.layer].color.background.main.push(
                    method.colorinvert(data.display.element[data.display.key].layer[variable.layer].color.background.main[data.display.element[data.display.key].layer[variable.layer].color.background.main.length - 1])
                );
                canvas.layer(variable.layer);
            },
        },
        ...
        },
        {
            ...
            value: (v) => {
                if (v != null) {
                    data.display.element[data.display.key].layer[variable.layer].effect.curve.type = v;
                    canvas.layer(variable.layer);
                }
                $(`[data-jd-pointer='text-effectcurvetype']`).val(data.display.element[data.display.key].layer[variable.layer].effect.curve.type);
        ...
        },
        {
            ...
            value: (v) => {
                ...
                if (data.display.element[data.display.key].layer[variable.layer].dimension.natural) {
                    data.display.element[data.display.key].layer[variable.layer].dimension.height =
                        (data.display.element[data.display.key].layer[variable.layer].dimension.width * data.display.element[data.display.key].layer[variable.layer].dimension.naturalheight) /
                        data.display.element[data.display.key].layer[variable.layer].dimension.naturalwidth;
                }
                canvas.layer(variable.layer);
            },
            render: () => {
        ...
        },
        {
            ...
            value: (v) => {
                ...
                if (data.display.element[data.display.key].layer[variable.layer].dimension.natural) {
                    data.display.element[data.display.key].layer[variable.layer].dimension.width =
                        (data.display.element[data.display.key].layer[variable.layer].dimension.naturalwidth * data.display.element[data.display.key].layer[variable.layer].dimension.height) /
                        data.display.element[data.display.key].layer[variable.layer].dimension.naturalheight;
                }
                canvas.layer(variable.layer);
            },
            render: () => {
        ...
        },
        {
            ...
            render: () => {
                ...
                data.display.element[data.display.key].layer[variable.layer].color.background.type != "normal" &&
                data.display.element[data.display.key].layer[variable.layer].color.background.type != "same-parent" &&
                data.display.element[data.display.key].layer[variable.layer].color.background.type != "none"
                    ? jd.pointer(`image-colorbackgroundadd`).css({ display: "block" })
                    : jd.pointer(`image-colorbackgroundadd`).css({ display: "none" });
                data.display.element[data.display.key].layer[variable.layer].color.background.type == "linear-gradient"
                    ? jd.pointer(`image-colorbackgroundposlinear`).css({ display: "block" })
                    : jd.pointer(`image-colorbackgroundposlinear`).css({ display: "none" });
                data.display.element[data.display.key].layer[variable.layer].color.background.type == "radial-gradient"
                    ? $(`[data-jd-pointer='image-colorbackgroundposrasialx'], [data-jd-pointer='image-colorbackgroundposrasialy']`).css({ display: "block" })
                    : $(`[data-jd-pointer='image-colorbackgroundposrasialx'], [data-jd-pointer='image-colorbackgroundposrasialy']`).css({ display: "none" });
            },
        },
        {
                ...
                const proses = {
                    ...
                    captures: () => {
                        $.each(save.captures, (i, val) => {
                            $.ajax({
                                url: `https://api.imgbb.com/1/upload?key=${variable.imgbb.key}&image=${encodeURIComponent(
                                    `https://api.apiflash.com/v1/urltoimage?access_key=${variable.apiflash.key}&url=${encodeURIComponent(val.address)}&format=${val.type == "mockup" ? "jpeg" : "png"}&width=${val.width}&height=${
                                        val.height
                                    }&fresh=true&full_page=true&response_type=image&transparent=true&wait_until=network_idle`
                                )}`,
                                method: "GET",
                                timeout: 0,
                                processData: false,
                                ...
                                $.each(data.display.position, (i, val) => {
                                    if (val.available) {
                                        ...
                                        if (data.display.element[i].layer.length != 0) {
                                            str += `{"type":"raw", "width":${variable.save.width}, "height":${
                                                (variable.save.width * val.printable.height) / val.printable.width
                                            }, "address": "https://joyo-design.github.io/capture.html?jsonbin_id=${save.jsonbinid}&mode=raw&position=${i}"},`;
                                        }
                                    }
                                });
        ...
        },
        {
            pointer: "data-import",
            value: (v) => {
                const files = document.querySelector(`[data-jd-pointer='data-import']`).files;
                if (files.length <= 0) {
                    return false;
                }
                const fr = new FileReader();
                fr.onload = function (e) {
                    ...
                    input.render();
                    method.unredo(data);
                    method.inputrender(`product-edit`, JSON.stringify(data));
                };
                fr.readAsText(files.item(0));
                $(`[data-jd-pointer='data-import']`).val(null);
            },
        },
        {
            pointer: "data-edit",
            ...
            render: () => {
                method.inputrender(`data-edit`, JSON.stringify(data));
            },
        },
        {
            pointer: "data-export",
            value: (v) => {
                method.export(data, "sample.json");
            },
        },
    ];
    ...
    const canvas = {
        ...
        printable: () => {
            $(`#jd-printable`).css({
                display: `${variable.printable ? "block" : "none"}`,
                width: `${data.display.position[data.display.key].printable.width}px`,
                height: `${data.display.position[data.display.key].printable.height}px`,
                border: `1px solid ${method.colorinvert(data.product.color.main)}`,
                left: `${data.display.position[data.display.key].printable.left}px`,
                top: `${data.display.position[data.display.key].printable.top}px`,
            });
        },
        element: () => {
            ...
            $.each(data.display.element[data.display.key].layer, function (i, v) {
                $("#jd-canvas-element").append(
                    `<div class="jd-canvas-element-${data.display.element[data.display.key].layer[i].type} jd-canvas-element-layer" data-jd-element-type="${
                        data.display.element[data.display.key].layer[i].type
                    }" data-jd-element-layer="${i}"></div>`
                );
                canvas.layer(i);
            });
        },
        layer: (n) => {
            ...
            const stroke = () => {
                ...
                $.each(pointer.outline, (i, v) => {
                    stroke += `drop-shadow(${v.x}${(data.display.element[data.display.key].layer[n].effect.stroke.width / 1000).toFixed(3)}em ${v.y}${(data.display.element[data.display.key].layer[n].effect.stroke.width / 1000).toFixed(
                        3
                    )}em 0em ${data.display.element[data.display.key].layer[n].effect.stroke.color}) `;
                });
            ...
            const outline = () => {
                let outline = ``;
                $.each(pointer.outline, (i, v) => {
                    outline += `drop-shadow(${v.x}${(data.display.element[data.display.key].layer[n].effect.outline.width / 1000).toFixed(3)}em ${v.y}${(data.display.element[data.display.key].layer[n].effect.outline.width / 1000).toFixed(
                        3
                    )}em 0em ${data.display.element[data.display.key].layer[n].effect.outline.color}) `;
                });
            ...
            const depth = () => {
                ...
                for (var l = 0; l <= data.display.element[data.display.key].layer[n].effect.depth.width; l++) {
                    depth += `drop-shadow(${pointer.depth[data.display.element[data.display.key].layer[n].effect.depth.type].x * l * 0.01}em ${
                        pointer.depth[data.display.element[data.display.key].layer[n].effect.depth.type].y * l * 0.01
                    }em ${l == 0 ? "1px" : "0em"} ${data.display.element[data.display.key].layer[n].effect.depth.color})`;
                }
            ...
            const color = () => {
                let color = `linear-gradient(0deg, ${data.display.element[data.display.key].layer[n].color.background.main[data.display.element[data.display.key].layer[n].color.background.main.length - 1]}, ${
                    data.display.element[data.display.key].layer[n].color.background.main[data.display.element[data.display.key].layer[n].color.background.main.length - 1]
                })`;
                let gradient = `${data.display.element[data.display.key].layer[n].color.background.main[data.display.element[data.display.key].layer[n].color.background.main.length - 1]}, ${
                    data.display.element[data.display.key].layer[n].color.background.main[data.display.element[data.display.key].layer[n].color.background.main.length - 1]
                }`;
            ...
            const shadow = `${
                data.display.element[data.display.key].layer[n].shadow.blur == 0 && data.display.element[data.display.key].layer[n].shadow.posx == 0 && data.display.element[data.display.key].layer[n].shadow.posy == 0
                    ? ``
                    : `drop-shadow(${(data.display.element[data.display.key].layer[n].shadow.posx / 100).toFixed(3)}em ${(data.display.element[data.display.key].layer[n].shadow.posy / 100).toFixed(3)}em ${(
                          data.display.element[data.display.key].layer[n].shadow.blur / 100
                      ).toFixed(3)}em ${data.display.element[data.display.key].layer[n].shadow.color})`
            }`;
            ...
            if (jd.layer(n).attr("data-jd-element-type") == "text") {
                ...
                const css = {
                    ...
                    transform: `rotate(${data.display.element[data.display.key].layer[n].transform.rotate}deg) skew(${data.display.element[data.display.key].layer[n].transform.skew}deg) scaleX(${
                        data.display.element[data.display.key].layer[n].transform.flip.vertical ? "-1" : "1"
                    }) scaleY(${data.display.element[data.display.key].layer[n].transform.flip.horizontal ? "-1" : "1"})`,
                        ...
                        .css({
                            borderBottom: `${
                                data.display.element[data.display.key].layer[n].text.underline
                                    ? `0.1em solid ${data.display.element[data.display.key].layer[n].color.background.main[data.display.element[data.display.key].layer[n].color.background.main.length - 1]}`
                                    : "none"
                            }`,
                    ...
                    const curve = {
                        text: {
                            start: JSON.parse(`${text}`)
                                .splice(0, JSON.parse(`${text}`).length % 2 == 1 ? JSON.parse(`${text}`).length / 2 - 0.5 : JSON.parse(`${text}`).length / 2)
                                .reverse(),
              ...
              <div style="transform: rotate(${re(i)}deg)">
                <div>
                  <div style="transform: rotate(${rpe(i)}deg)">
                    ...
                    $(`.jd-canvas-element-layer[data-jd-element-layer="${n}"] > div > div`)
                        .css(addcss)
                        .css({ height: `${Number(curve.data.round / 100).toFixed(3)}em` })
                        .css(curve.pointer[curve.data.type].ps, "0px");
                ...
                const css = {
                    ...
                    height: `${
                        data.display.element[data.display.key].layer[n].dimension.natural
                            ? `${
                                  (data.display.element[data.display.key].layer[n].dimension.width * data.display.element[data.display.key].layer[n].dimension.naturalheight) /
                                  data.display.element[data.display.key].layer[n].dimension.naturalwidth
                              }`
                            : `${data.display.element[data.display.key].layer[n].dimension.height}`
                    }px`,
                    transform: `rotate(${data.display.element[data.display.key].layer[n].transform.rotate}deg) skew(${data.display.element[data.display.key].layer[n].transform.skew}deg) scaleX(${
                        data.display.element[data.display.key].layer[n].transform.flip.vertical ? "-1" : "1"
                    }) scaleY(${data.display.element[data.display.key].layer[n].transform.flip.horizontal ? "-1" : "1"})`,
                    filter: `${
                        data.display.element[data.display.key].layer[n].color.edit.mode ? `opacity(0.9) grayscale(100%) drop-shadow(0em 0em 0em ${data.display.element[data.display.key].layer[n].color.edit.main}) saturate(100)` : ``
                    } ${data.display.element[data.display.key].layer[n].color.filter} ${stroke()} ${outline()} ${depth()} ${shadow} opacity(${(data.display.element[data.display.key].layer[n].color.opacity / 100).toFixed(2)})`,
                    borderRadius: `${data.display.element[data.display.key].layer[n].round.top.left}% ${data.display.element[data.display.key].layer[n].round.top.right}% ${
